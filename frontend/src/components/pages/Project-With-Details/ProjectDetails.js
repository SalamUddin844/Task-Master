...
const ProjectDetails = () => {
  ...
  const fetchSprints = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`http://localhost:5001/api/sprints/project/${projectId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  // Filter & search sprints
  useEffect(() => {
    let temp = [...sprints];

    if (filterStatus !== "all") {
      temp = temp.filter((s) => (s.status || "").toLowerCase() === filterStatus.toLowerCase());
    }

    if (searchText.trim() !== "") {
      temp = temp.filter((s) => s.title.toLowerCase().includes(searchText.toLowerCase()));
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5001/api/sprints/${sprintToDelete.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
